...
function loadPage() {
    ...
    request.open("get", "http://demo4257344.mockable.io/product.json", true);
    request.onload = () => {
        try {
            var items = JSON.parse(request.responseText);
            populateItems(items); //function call to render the initial output template
            // init();
        } catch (e) { //call back function to catch the error and display the same to the user if required
...
function populateItems(items) { // populate data
    // console.log(items);
    for (var i in items) { //looping over the items in the JSON API 
        ...
        var delivery_status = delivery_charge != 0 ? "RS." + delivery_charge : "FREE"; //ternary operator to validate if delivery is free or not
    ...
    document.querySelector("#total_price").innerHTML = `Rs. ${total}`; // updates initial value of total price
    document.querySelector("#delivery_charges").innerHTML = `Rs. ${delivery}`; // updates initial value of delivery charges
    document.querySelector("#amount_payable").innerHTML = `Rs. ${payable}`; // updates initial value of amount payable
}
document.addEventListener("DOMContentLoaded", () => { //call the loadPage function once DOM, images and other resources are fully rendered on the page 
    loadPage(items);
});
...
var removeFromCart = function decrease(index) { // function to remove 1 qty from cart.
    if (cart_item[index] > 0) {
        ...
        document.getElementsByClassName("item_num_value")[index].childNodes[0].nodeValue = cart_item[index]; //update the value of the counter to DOM
...
var addToCart = function increase(index, max) { // function to add 1 qty from cart.
    if (cart_item[index] < max) { // condition for the max item availability. 
...
function update(index, ch) {
    fetch("http://demo4257344.mockable.io/product.json")
        ...
        .then(arr => {
            ...
            document.querySelector("#total_price").innerHTML = `Rs. ${total}`;
            document.querySelector("#delivery_charges").innerHTML = `Rs. ${delivery}`;
            document.querySelector("#amount_payable").innerHTML = `Rs. ${payable}`;
        });
    ...
    fetch("http://demo4257344.mockable.io/product.json")
        ...
        .then(data => {
            ...
            for (let item of data) {
                let dummy = item.availability.unavailable_pincodes;
